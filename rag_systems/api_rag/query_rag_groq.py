"""
Query RAG từ database được index bởi Groq
Vẫn dùng Local Llama 3.1 để query (bảo mật, miễn phí)

Chạy: uv run query_rag_groq.py
Hoặc: uv run query_rag_groq.py "câu hỏi"
"""
...
# ======================== PATHS (khớp với index_docs_groq.py) ========================
RAG_STORAGE = "./rag_storage_groq"
...
# ======================== MODELS SETUP ========================
def setup_models():
    """Load local models cho query (không cần Groq)"""
    ...
    async def initialize(self):
        ...
        if not os.path.exists(RAG_STORAGE):
            raise FileNotFoundError(
                ...
                f"   Chạy 'uv run index_docs_groq.py' để index trước!"
            )
        ...
        if not os.path.exists(vdb_file):
            raise FileNotFoundError(
                f"❌ Database rỗng.\n"
                f"   Chạy 'uv run index_docs_groq.py' để index!"
            )
        
        # Setup
        embedding, llm_func = setup_models()
        
        print("\n🔧 Loading RAG database (Groq-indexed)...")
...
async def interactive_mode(querier: RAGQuerier):
    print("\n" + "=" * 60)
    print("💬 CHẾ ĐỘ HỎI ĐÁP (Groq Database + Local LLM)")
    print("=" * 60)
...
async def main_async(args):
    if args.question:
        ...
        print("=" * 60)
        print("🚀 RAG QUERIER (Groq Database + Local LLM)")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Query Groq-indexed RAG")
